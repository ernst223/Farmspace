...
import { DeleteByIdDto, ProblemForDisplayDto } from 'app/shared/shared.models';
...
@Component({
  ...
  styleUrls: ['./upload-solution.component.scss']
})
export class UploadSolutionComponent implements OnInit {
  ...
  myData: ProblemForDisplayDto[];
  myLatestUploadedData: ProblemForDisplayDto[];
  uploadAmount = 0;
  isDeleteing = false;
  DeleteEntriesDto: DeleteByIdDto = {id: []};
  displayHeading = '';
  orderDataSource: MatTableDataSource<object> = new MatTableDataSource();
  displayedColumns = ['Id', 'ProblemName', 'Select'];
  ...
  setupDataStream(){
    ...
    this.displayHeading = "Latest 100 Problems";
    this.share.getProblemForDisplay().subscribe(a => {
      this.myData = a;
  ...
  setLatestUploadedData() {
    this.displayHeading = "Uploaded Problems";
    if(this.uploadAmount == 0){
      this.openSnackBar('No Uploaded Data', 'Close');
      this .orderDataSource.data = undefined;
    }else{
      this.share.getProblemForDisplayAmount(this.uploadAmount).subscribe(a => {
  ...
  uploadEntry(){
    ...
    this.share.uploadProblem(this.theFile).subscribe(a => {
      this.loader = false;
      ...
      if(this.selectAll == true) {
        ...
        if(this.showLatestUploads == true){
          this.myLatestUploadedData.forEach(element => {
            this.DeleteEntriesDto.id.push(element.problemBaseId);
          });
        }else{
